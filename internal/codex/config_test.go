...
func TestWriteConfigAddsTheProviderAndKeepsTheRest(t *testing.T) {
	...
	if err := WriteConfig(home, "http://127.0.0.1:38440/", ""); err != nil {
		t.Fatal(err)
	}
	first, _ := os.ReadFile(filepath.Join(home, "config.toml"))
	if !strings.Contains(string(first), `base_url = 'http://127.0.0.1:38440/v1'`) || !strings.Contains(string(first), `wire_api = 'responses'`) {
		t.Fatalf("config:\n%s", first)
	}
	extra := string(first) + "\n[projects.\"/x\"]\ntrust_level = 'trusted'\n"
	...
	if err := WriteConfig(home, "http://127.0.0.1:39000", ""); err != nil {
		t.Fatal(err)
	}
